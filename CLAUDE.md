...
- `index.html` — the homepage: nav, hero with `.product-cards`, Plateful section, `.section-patchlamp` band, services teaser, short about, contact, footer.
...
- `assets/icons.svg` — the shared icon sprite (`<use href="assets/icons.svg#icon-github">`; `../assets/...` from a subfolder). `assets/og.png` is the share card (both marks); `assets/exchange-room-*` are Patchlamp's brand images, copied from the patchlamp repo.
...
- **Light theme** (redesigned 2026-08-27) everywhere except the scoped Patchlamp surfaces (the hero's Patchlamp card, the `#patchlamp` band, the `/ai/` panel), which use the `--pl-*` tokens and IBM Plex on purpose — the band should look like it came from a different building. White bg, `#f7f9fc` alt sections, ink `#101828`, teal accent `#0d9488` (`--accent-ink` `#0f766e` for accent-colored text). Use the CSS variables in `:root`, never hardcode new colors. Plateful brand colors (`--plateful` `#069494`, `--plateful-red` `#b22222`) are reserved for the Plateful section.
...
- Never invent testimonials, client names, or metrics. Every claim on the site must be true (Plateful is live; the stats on `/resume/` come from Taylor's résumé).
...
`sms-relay/`, and each Plateful claim in `plateful/`.
